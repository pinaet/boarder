...
class RegisterColumn extends Model
{
    ...
    public function get_start_date($dates, $weekly=false)
    {
        //get Monday date of the week
        if( $weekly==false ){
            $dates = (new BoarderController)->generate_dates( $dates[0]['formatted'], true );
        }
        $monday_date = $dates[0]['formatted'];

        //get list of start_date from RegisterColumn
        $start_dates = $this->select('start_date')->groupBy('start_date')->orderBy('start_date', 'desc')->get();
        $start_date  = '';
        foreach( $start_dates as $data ) 
        {
            if($data->start_date<=$monday_date){
